...
public:
    ...
    _laserScan_subscriber = this->create_subscription<sensor_msgs::msg::LaserScan>(lidarscan_topic, 10,
            [this](const sensor_msgs::msg::LaserScan::SharedPtr msg) {this->lidar_callback(msg);});
...
private:
    ...
    {   
        ...
        {
            ranges[i] = (ranges[i]+ranges[i-1]+ranges[i-2]+ranges[i-3]+ranges[i+1]+ranges[i+2]+ranges[i+3])/7.;

        if(ranges[i]>10) ranges[i] = 10; 
        }
        return;
    }
