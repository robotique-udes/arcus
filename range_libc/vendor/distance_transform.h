...
namespace dt
{
    ...
    class MArray
    {
      public:
        ...
        /**
         *    @brief Reorders the sub-matrixes s.t. the one at 0 <= i < D goes to 0 <= order[i] < D, i.e. m[*]..[*]_i...[*] swaps
         * with m[*]...[i]...[*]_order[i]...[*].
         *    @param order              A permutation of the matrix indices.
         *    @param p                  The output permuted matrix.
         *    @note Example: transpose a 2D matrix by swapping the access indices - MArray<T,2> m, mt; std::size_t trans_ord[2] =
         * {1, 0}; m.permute(trans_ord, mt);
         */
      ...
      private:
        ...
        std::size_t
            _offset[D];  ///< Jumps' offsets from the beginning of a "row" to the beginning of the next one, for each dimension.
    };
...
namespace dt
{
    ...
    /// sampled function. The dimension is fixed at compile time. It is also possible to compute the index of the nearest minimum
    /// of each sample.
    class DistanceTransform
    {
      public:
        /**
         *    @brief Compute the L2-norm distance field D of a DIM-dimensional sampled function f. D gets the distance from the
         * local minima of f.
         ...
         */
        template<typename Scalar = float, std::size_t DIM = 2>
        inline static void distanceTransformL2(const MMArray<Scalar, DIM>& f,
                                               MMArray<Scalar, DIM>& D,
                                               MMArray<std::size_t, DIM>& I,
         ...
         */
        template<typename Scalar = float>
        inline static void distanceTransformL2(const MMArray<Scalar, 1>& f,
                                               MMArray<Scalar, 1>& D,
                                               MMArray<std::size_t, 1>& I,
                                               const bool squared = false)
        {
            std::size_t size[1];
            f.size(size);
      ...
      private:
        ...
        /**
         *    @brief The actual distance field computation as in the "Distance Transforms of Sampled Functions" paper, performed
         * in a mono-dimensional function.
         *    @param f              A 1-dimensional, regularly sampled function.
         *    @param D              The resulting distance field of f.
         */
        template<typename Scalar = float>
         ...
         */
        template<typename Scalar = float, std::size_t DIM>
        inline static void distanceL2(const MArray<Scalar, DIM>& f,
                                      MArray<Scalar, DIM>& D,
                                      const MArray<std::size_t, DIM>& Ipre,
                                      MArray<std::size_t, DIM>& Ipost)
        {
            MArray<Scalar, DIM - 1> f_q, D_q;
         ...
         */
        template<typename Scalar = float>
        inline static void distanceL2(const MArray<Scalar, 1>& f,
                                      MArray<Scalar, 1>& D,
                                      const MArray<std::size_t, 1>& Ipre,
