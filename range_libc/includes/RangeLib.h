...
/*
Useful Links:
https://github.com/MRPT/mrpt/blob/4137046479222f3a71b5c00aee1d5fa8c04017f2/libs/slam/include/mrpt/slam/PF_implementations.h
...
// this stuff is for ROS integration, not necessary for raw usage
    ...
    class RangeMethod
    {
      public:
        ...
        void calc_range_repeat_angles_eval_sensor_model(float* ins,
                                                        float* angles,
                                                        float* obs,
                                                        double* weights,
                                                        int num_particles,
        ...
        void calc_range_many_radial_optimized(float* ins,
                                              float* outs,
                                              int num_particles,
                                              int num_rays,
                                              float min_angle,
    ...
    class RayMarchingGPU : public RangeMethod
    {
      public:
...
#if ROS_WORLD_TO_GRID_CONVERSION == 1
            rmc->set_conversion_params(m.world_scale,
                                       m.world_angle,
                                       m.world_origin_x,
                                       m.world_origin_y,
                                       m.world_sin_angle,
        ...
        void maybe_warn(int num_casts)
        {
            ...
            {
                std::cout << "\nFor better performance, call calc_range_many with some multiple of " << CHUNK_SIZE
                          << " queries. ";
        ...
        void numpy_calc_range_angles(float* ins, float* angles, float* outs, int num_particles, int num_angles)
        {
            ...
            for (int i = 0; i < iters; ++i)
            {
                ...
                rmc->numpy_calc_range_angles(&ins[i * num_in_chunk * 3],
                                             angles,
                                             &outs[i * num_in_chunk * num_angles],
                                             num_in_chunk,
                                             num_angles);
            }
#else
                                                        ...
                                                        int num_angles)
        {
            ...
            for (int i = 0; i < iters; ++i)
            {
                ...
                if (i == iters - 1)
                    num_in_chunk = num_particles - i * particles_per_iter;
                rmc->calc_range_repeat_angles_eval_sensor_model(&ins[i * num_in_chunk * 3],
                                                                angles,
                                                                obs,
...
// this is the y-coordinate for each non-origin corner
                    ...
                    if (edge_map.isOccupied(x, y))
                    {
                        ...
                        for (int a = 0; a < theta_discretization / 2.0; ++a)
                        {
                            ...
                            float lut_space_center_y
                                = (pixel_center.first * sinangle + pixel_center.second * cosangle) + lut_translations[a];
                            ...
                            // std::cout << "new center: (" << lut_space_center_x << ", " << lut_space_center_y << ")" <<
                            // std::endl; std::cout << "bins:" << upper_bin << "    " << (int) lut_space_center_y << "   " <<
                            // lower_bin << std::endl; std::cout << "width:" << half_lut_space_width << std::endl; std::cout <<
                            // "trans" << lut_translations[a] << std::endl; std::cout << upper_bin << "   " << lower_bin << "   "
                            // << lut_translations[a] << std::endl; std::cout << lut_space_center_x << "  " << lut_space_center_y
                            // << std::endl;
                        }
                    }
                }
            ...
            for (int a = 0; a < theta_discretization; ++a)
            {
                for (int i = 0; i < compressed_lut[a].size(); ++i)
                {
                    ...
                    compressed_lut[a][i].erase(unique(compressed_lut[a][i].begin(), compressed_lut[a][i].end()),
        ...
        int memory()
        {
            return lut_size() + map.memory() + lut_translations.size() * sizeof(float);
        }
            ...
            for (int angle_index = 0; angle_index < theta_discretization / 2.0; ++angle_index)
            {
                ...
                for (int x = 0; x < map.grid.size(); ++x)
                {
                    ...
                    for (int y = 0; y < map.grid[0].size(); ++y)
                    {
                        ...
                        {  // do linear search if array is very small
...
// if (cache.exists(key)) { return cache.get(key); }
        ...
        void serializeYaml(std::stringstream* ss)
        {
            ...
            (*ss) << T1 << "map: " << std::endl;
            (*ss) << T2 << "# note: map data is width and then height (width is number of rows) transposed from expectation:"
                  << std::endl;
        ...
        void serializeJson(std::stringstream* ss)
        {
            ...
            (*ss) << J1 << "\"map\": {" << std::endl;
            // (*ss) << J2 << "# note: map data is width and then height (width is number of rows) transposed from expectation:"
            // << std::endl;
...
namespace benchmark
{
    ...
    class Benchmark
    {
      public:
        ...
        void set_log(std::stringstream* ss)
        {
            log = ss;
        }
