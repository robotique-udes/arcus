...
// mark.getMap()->saveTrace(FLAGS_trace_path);
    ...
    if (utils::has("CDDTCast", methods) || utils::has("cddt", methods) || utils::has("PrunedCDDTCast", methods)
        || utils::has("pcddt", methods))
    {
        auto construction_start = std::chrono::high_resolution_clock::now();
...
#else
        std::cout << "\nNot compiled with CUDA enabled, please enable flag -DWITH_CUDA=ON to run RayMarchingGPU benchmarks."
                  << std::endl;
#endif
    }
